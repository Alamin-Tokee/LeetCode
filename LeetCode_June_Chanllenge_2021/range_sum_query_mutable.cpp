...
// 2. Another way would be solve using Segment Trees which would do the SUM in O(logn) and UPDATE in O(logn) time. Howeever, weee will have to build a Segment Tree first which will require O(n) time.
